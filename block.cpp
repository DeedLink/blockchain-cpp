...
Block::Block(uint64_t idx, const std::string& prevHash, const std::string& time,
             const std::string& blockData, const std::string& blockHash, uint64_t blockNonce,
             const std::string& signature, uint64_t blockStake)
...
std::string Block::calculateMerkleRoot() const {
    ...
    for (const auto& tx : transactions) hashes.push_back(tx.hash());
